...
function Orders({ user, socket }) {
  ...
  const add = new Audio(
    "https://cdn.pixabay.com/audio/2022/04/05/audio_c5c228d922.mp3"
  );
  ...
  useEffect(() => {
    ...
    socket.on("chat", async (chat) => {
      ...
      if (
        chat.message.toLowerCase().split(" ").includes("gay") ||
        chat.message.toLowerCase().split(" ").includes("🌈") ||
        chat.message.toLowerCase().split(" ").includes("🏳️‍🌈")
      ) {
        await gay.play();
      }
      scrollToBottom();
    ...
    const fetchOrders = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.PROD ? import.meta.env.VITE_SERVER_URL : import.meta.env.VITE_LOCAL_URL}/api/getOrders`
        );
        if (response.data) {
  ...
  const Typing = async (name) => {
    await axios.post(`${import.meta.env.PROD ? import.meta.env.VITE_SERVER_URL : import.meta.env.VITE_LOCAL_URL}/api/type/`, {
      name: name,
      id: socket.id,
    });
  };
  ...
  const addOrder = async () => {
    try {
      const { data } = axios.post(
        `${import.meta.env.PROD ? import.meta.env.VITE_SERVER_URL : import.meta.env.VITE_LOCAL_URL}/api/addOrder`,
        {
          status: "cooking",
          client_id: socket.id,
        }
  ...
  const Chat = async () => {
    if (newChat == "") return;
    await axios.post(`${import.meta.env.PROD ? import.meta.env.VITE_SERVER_URL : import.meta.env.VITE_LOCAL_URL}/api/chat`, {
      id: socket.id,
      message: newChat,
  ...
  const prepareOrder = async (id) => {
    try {
      const { data } = await axios.post(
        `${import.meta.env.PROD ? import.meta.env.VITE_SERVER_URL : import.meta.env.VITE_LOCAL_URL}/api/prepareOrder`,
        {
          _id: id,
        }
      );
  ...
  const deliverOrder = async (id) => {
    try {
      const { data } = await axios.post(
        `${import.meta.env.PROD ? import.meta.env.VITE_SERVER_URL : import.meta.env.VITE_LOCAL_URL}/api/deliverOrder`,
        {
          _id: id,
        }
      );
  ...
  const alertUser = async (user) => {
    try {
      console.log(user);
      const { data } = await axios.post(
        `${import.meta.env.PROD ? import.meta.env.VITE_SERVER_URL : import.meta.env.VITE_LOCAL_URL}/api/alertAll`,
        {
          id: user,
        }
      );
  ...
  return (
    <div className="flex flex-col items-center flex-grow bg-gray-200 min-w-screen">
      <div className="flex flex-col gap-2 md:w-[90%] m-2 justify-center items-center min-h-[5rem] ">
        <h1 className="md:text-[1.2rem] text-[0.8rem]">
          Currently Online: {Object.keys(online).length}
        </h1>
          ...
          <div className="flex flex-col items-center justify-center w-[100%] md:w-[60%] gap-2">
            {Object.keys(online).map((user) => {
              ...
              return (
                <div
                  key={user}
                  className="flex flex-grow justify-stat  items-center gap-2 w-[100%] md:w-[60%]"
                >
                  <h1 className="md:text-[1.2rem]  relative text-[0.8rem]">
                    {online[user] != "anonymous" || online[user] != ""
                      ? online[user]
                      : user}
                  ...
                  <div className="flex justify-end flex-grow">
                    <button
                      ...
                      className="p-2 text-white bg-green-500 rounded-md shadow-md"
                    >
                      Notify
                    </button>
                  </div>
      ...
      {ready.status && (
        <div className="bg-green-200 w-[90%] p-2 m-2 animate-pulse rounded-md outline outline-2 outline-green-500">
          <h1 className="md:text-[1.3rem] text-[0.8rem] font-semibold text-green-700">
            Order id: {ready.id} ready
          </h1>
        </div>
      )}
      ...
      <div className=" mb-10 mt-10 w-[90%] p-2 gap-2 flex flex-col rounded-md shadow-md bg-gradient-to-tr from-yellow-200 to-slate-200">
        {user && (
          <button
            ...
            className="p-2 mt-5 text-white bg-green-500 rounded-md shadow-md"
          >
            Add Order
          </button>
        )}
        {user ? (
          orders.map((order) => {
            return (
              <div
                key={order._id}
                className="flex items-center gap-4 p-2 bg-white/[40%] rounded-md shadow-md backdrop-blur-md"
              >
                <h1>${order._id}</h1>
                <div className="flex justify-end flex-grow gap-2 ">
                  <button
                    onClick={() => {
                      prepareOrder(order._id);
                    }}
                    disabled={order.status == "ready" ? true : false}
                    className="p-2 active:scale-90 disabled:active:scale-100 transition-all md:text-[1rem] text-[0.8rem] font-semibold text-white bg-green-500 rounded-full shadow-md disabled:bg-green-300"
                  >
                    Ready
                  </button>
                  <button
                    ...
                    className="p-2 font-semibold text-white bg-red-500 md:text-[1rem] transition-all disabled:active:scale-100 active:scale-90 text-[0.8rem] rounded-full shadow-md"
                  >
                    Delete
                  </button>
                </div>
          ...
          <div className="flex flex-col items-center justify-center flex-grow">
            <h1 className="text-lg font-semibold md:text-2xl animate-pulse">
              Login to see and place orders
            </h1>
          </div>
        )}
        <div className="flex flex-col gap-2 mt-5">
          <div className="text-[1.6rem] relative w-max   font semibold">
            ...
            <div>
              Live Chatrooom
              <h1 className="md:text-[0.8rem] w-max p-1 font-semibold backdrop-blur-sm text-white bg-green-500/[99%] text-center shadow-md text-[0.4rem]  rounded-full">
                Currently Online: {Object.keys(online).length}
              </h1>
            </div>
          </div>
              ...
              {typing && (
                <div className="sticky m-0 -top-2 bottom-0 flex justify-end w-[100%]">
                  <div className="w-[100%] flex justify-end z-1 text-green-500  p-2 font-semibold bg-white/[90%] backdrop-blur-md shadow-md rounded-b-lg">
                    <h1 className="animate-pulse">{typing} is typing.....</h1>{" "}
                  </div>
                </div>
              )}
              {chats?.map((chat) => {
                  ...
                  chat?.id === socket?.id ? (
                      ...
                      {chat.isMember ? (
                        chat.message
                          ...
                          <div className="overflow-hidden bg-red-500 rounded-b-lg">
                            <img
                              src="https://media.tenor.com/NkfNBoWQnBMAAAAM/superstarksa-po-po.gif"
                              alt=""
                            />
                          </div>
                        ) : chat.message
                          ...
                          <div className="overflow-hidden rounded-lg max-h-[16em] max-w-[15rem] rounded-b-lg">
                            <img
                              src="https://media1.tenor.com/m/KkerOljBwakAAAAd/computer-nerd.gif"
                              alt=""
                              className="w-[100%] h-[100%]"
                            />
                          </div>
                          ...
                          <h1 className="w-max min-w-[12rem] max-w-[8rem] text-white text-wrap p-2 rounded-lg bg-blue-400">
                            {chat.message}
                          </h1>
                        )
                      ) : (
                    ...
                    <div className="flex flex-col w-full -z-10 " key={chat.id}>
                      <div
                        className={`${
                          chat.isMember
                            ? "flex flex-col bg-gradient-to-tr from-blue-500  to-green-500   rounded-lg shadow-md backdrop-blur-sm w-max"
                            : "flex flex-col bg-green-400  rounded-lg shadow-md backdrop-blur-sm w-max"
                        }`}
                      >
                        <div className="bg-gradient-to-tr flex items-center gap-1 rounded-t-lg from-yellow-500 to-violet-200 min-w-[8rem] p-2 ">
                          <h1 className="text-[0.8rem] text-white font-semibold">
                            {chat.name ? chat.name : chat.id}
                          </h1>
                          {chat.isMember && (
                            <svg
                              aria-label="Verified"
                              className="scale-[75%] x1lliihq x1n2onr6"
                              fill="rgb(0, 149, 246)"
                              height="18"
                              role="img"
                              viewBox="0 0 40 40"
                        ...
                        {chat.isMember ? (
                          chat.message
                            ...
                            <h1 className="w-max max-w-[12rem] min-w-[8rem] text-wrap text-white p-2 ">
                              {chat.message}
                            </h1>
                          )
                        ) : (
            ...
            <div className="flex pt-2">
              <input
                ...
                onChange={(e) => {
                  ...
                  Typing(user && user.name ? user.name : socket.id);
                }}
